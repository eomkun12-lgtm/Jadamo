...
export default function NoticeCenter() {
  ...
  return <>
    <div className="notice-shortcuts">
      <button className="notice-admin-button" type="button" onClick={openManager}>{isAdmin ? "공지 관리" : "관리자"}</button>
      <button className="home-screen-guide-button" type="button" onClick={() => setHomeGuideOpen(true)}>홈 화면 추가</button>
    </div>
    {homeGuideOpen && <div className="notice-popup home-screen-guide" role="dialog" aria-modal="true" aria-labelledby="home-screen-guide-title"><button className="notice-popup-scrim" onClick={() => setHomeGuideOpen(false)} aria-label="홈 화면 추가 안내 닫기" /><article><div className="notice-popup-label">SAFARI QUICK ACCESS</div><button className="notice-popup-close" onClick={() => setHomeGuideOpen(false)} aria-label="닫기">×</button><h2 id="home-screen-guide-title">홈 화면에 추가하기</h2><p>Safari에서 이 페이지를 열고 아래 순서로 추가해 주세요.</p><ol className="home-screen-guide-steps"><li><b>공유</b> 버튼을 누르세요.</li><li>메뉴에서 <b>홈 화면에 추가</b>를 선택하세요.</li><li><b>추가</b>를 누르면 홈 화면에서 바로 열 수 있습니다.</li></ol><div className="notice-popup-actions"><button onClick={() => setHomeGuideOpen(false)}>확인</button></div></article></div>}
