...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (action === "create-calendar" || action === "sync") {
      ...
      const failed = result.failures.length ? ` ${result.failures.length}개 일정은 실패했습니다: ${result.failures.slice(0, 2).join(" / ")}` : "";
      return Response.json({ message: `${result.synced}개 일정을 동기화했습니다.${result.skipped ? ` 날짜 미정 ${result.skipped}개는 제외했습니다.` : ""}${failed}`, partialFailure: result.failures.length > 0 });
    }
    ...
    const message = error instanceof Error ? error.message : "Google Calendar 요청을 처리하지 못했습니다.";
