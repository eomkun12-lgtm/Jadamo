...
export default function DjiPhotoImporter({ destinationId, logs, onUploaded }: { destinationId: string; logs: DiveLog[]; onUploaded: () => Promise<void> | void }) {
  const [open, setOpen] = useState(false); const [items, setItems] = useState<Item[]>([]); const [status, setStatus] = useState(""); const [uploading, setUploading] = useState(false); const [uploadedCount, setUploadedCount] = useState(0); const itemsRef = useRef<Item[]>([]);
  ...
  async function upload() {
    if (!assigned.length || uploading) return; setUploading(true); setUploadedCount(0); setStatus("사진을 올리고 다이브 로그에 연결하고 있습니다.");
    try { const nextUrls = new Map(logs.map((log) => [log.id, [...log.photoUrls]]));
      for (let index = 0; index < assigned.length; index += 1) { const item = assigned[index]; setStatus(`${index + 1} / ${assigned.length}번째 사진을 올리고 있습니다.`); const form = new FormData(); form.set("destinationId", destinationId); form.set("category", "ocean"); form.set("caption", `${item.capturedAt.toLocaleString("ko-KR")} 촬영`); form.set("file", await resize(item.file)); const response = await fetch("/api/underwater-photos", { method: "POST", body: form }); const data = await response.json() as { photo?: { imageUrl: string }; error?: string }; if (!response.ok || !data.photo) throw new Error(data.error || "사진 업로드에 실패했습니다."); const imageUrl = `${data.photo.imageUrl}?capturedAt=${encodeURIComponent(item.capturedAt.toISOString())}`; const urls = nextUrls.get(item.logId!) || []; if (!urls.includes(imageUrl)) urls.push(imageUrl); nextUrls.set(item.logId!, urls); setUploadedCount(index + 1); }
  ...
  const secondaryActionStyle = { minHeight: 42, padding: "0 16px", border: "1px solid #9db6bc", borderRadius: 9, color: "#24586a", background: "#fff", boxShadow: "0 1px 0 rgba(22, 59, 74, .08)", fontWeight: 750, cursor: uploading ? "wait" : "pointer" };
  const primaryActionStyle = { minHeight: 42, padding: "0 16px", border: "1px solid #a9452b", borderRadius: 9, color: "#fff", background: "linear-gradient(135deg, #e37244, #c64d30)", boxShadow: "0 4px 10px rgba(186, 70, 38, .22)", fontWeight: 800, cursor: !assigned.length || uploading ? "not-allowed" : "pointer" };
  return <>
    <button type="button" onClick={() => setOpen(true)} style={{ background: "linear-gradient(135deg, #e37244, #c64d30)", color: "#fff", border: 0, borderRadius: 10, padding: "12px 17px", fontWeight: 800, letterSpacing: ".02em", boxShadow: "0 8px 18px rgba(186, 70, 38, .27)", display: "inline-flex", gap: 8, alignItems: "center" }}><span aria-hidden="true" style={{ fontSize: 18, lineHeight: 1 }}>↑</span> 사진 분류</button>
    {open && <div role="presentation" onMouseDown={() => !uploading && setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(8, 24, 32, .68)", display: "grid", placeItems: "center", padding: 18 }}>
      <section role="dialog" aria-modal="true" aria-label="사진 분류" onMouseDown={(event) => event.stopPropagation()} style={{ width: "min(920px, 100%)", maxHeight: "86vh", overflow: "auto", background: "#f8f6f0", color: "#163b4a", borderRadius: 18, padding: "24px clamp(16px,4vw,36px)", boxShadow: "0 24px 70px rgba(0,0,0,.3)" }}>
        ...
        {uploading && <div aria-live="polite" style={{ margin: "18px 0 0" }}><div style={{ display: "flex", justifyContent: "space-between", fontWeight: 800, color: "#24586a", fontSize: 13 }}><span>업로드 진행률</span><span>{Math.round((uploadedCount / assigned.length) * 100)}%</span></div><div style={{ marginTop: 7, height: 10, overflow: "hidden", borderRadius: 99, background: "#d7e4e1" }}><div style={{ height: "100%", width: `${Math.round((uploadedCount / assigned.length) * 100)}%`, borderRadius: 99, background: "linear-gradient(90deg, #e37244, #f4aa59)", transition: "width .3s ease" }} /></div></div>}
        {status && <p role="status" style={{ margin: "12px 0 0", color: "#376674" }}>{status}</p>}
        <footer style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 20 }}><small>{assigned.length}장 연결 예정</small><div style={{ display: "flex", gap: 8, padding: 5, border: "1px solid #d3e0df", borderRadius: 12, background: "#edf3f1" }}><button type="button" onClick={() => setOpen(false)} disabled={uploading} style={{ ...secondaryActionStyle, opacity: uploading ? .55 : 1 }}>취소</button><button type="button" onClick={() => void upload()} disabled={!assigned.length || uploading} style={{ ...primaryActionStyle, opacity: !assigned.length || uploading ? .5 : 1 }}>{uploading ? "업로드 중…" : `${assigned.length}장 로그에 올리기`}</button></div></footer>
      </section>
    </div>}
  </>;
}
